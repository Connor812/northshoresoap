...
function Cart() {
   ...
   const { cartItems, setCartItems, subTotal, setSubtotal, hst, setHst, grandTotal, setGrandTotal } = useContext(DataContext);
                        ...
                        <div className="cart-items">
                           ...
                           {cartItems.length === 0 ? (
                              ...
                              cartItems.map((item, index) => {
                                 return (
                                    <CartItem key={index} item={item} index={index} removeItem={removeItem} updateQuantity={updateQuantity} setCartItems={setCartItems} setSubtotal={setSubtotal} setHst={setHst} setGrandTotal={setGrandTotal} />
                                 )
                              })
                           )}
                        </div>
